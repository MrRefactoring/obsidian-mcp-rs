...
npx obsidian-mcp-rs update     # взять последний релиз сразу, не дожидаясь суточной проверки
...
- **Ходит в сеть не чаще раза в сутки**, сколько бы раз клиент ни перезапускал сервер, а неудачная проверка проходит молча — машина без сети или за файрволом от неё не тормозит и не ломается.
...
Выключить — при установке или в любой момент после:

```bash
npx obsidian-mcp-rs@latest install --no-auto-update
```

Интерактивный визард спрашивает, а не решает за вас, и по умолчанию предлагает текущий выбор. Ответ запоминается; передумать — `install --auto-update`. Установка, которая про авто-обновление ничего не говорит, ничего и не меняет: настройка второго клиента не включит его обратно.

Взять релиз сразу, не дожидаясь проверки:

```bash
obsidian-mcp-rs update           # взять последний релиз
obsidian-mcp-rs update --check   # просто сказать, что доступно
obsidian-mcp-rs update --force   # взять, не дожидаясь 48-часовой выдержки
```

**Что уходит при проверке.** Один `GET` на `api.github.com` в сутки — с тем, что несёт любой HTTP-запрос, то есть с вашим IP, и с `User-Agent` вида `obsidian-mcp-rs/<версия>`. Само обновление затем качает бинарник с `objects.githubusercontent.com`. Ничего о ваших хранилищах, заметках, клиенте или машине не передаётся, и ничто не связывает вас между днями сверх того, что GitHub может вывести из адреса. Переменные `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` учитываются. Если и это лишний запрос — `--no-auto-update` выключает его полностью.

Вернуться на более старый релиз — поставить его: `npx obsidian-mcp-rs@0.7.1 install`. Путь в конфигах не меняется никогда, поэтому перенастраивать ничего не нужно ни в ту, ни в другую сторону. (Одно исключение, и оно касается всех, кто ставился рано: в конфиге, созданном до 0.7.0, этого пути ещё нет, так что до него ничего из перечисленного не доходит — см. [ниже](#обновление-конфига-созданного-до-070).)
