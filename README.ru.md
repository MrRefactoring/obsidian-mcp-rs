...
> Установи MCP-сервер **obsidian-mcp-rs** для этого редактора. Мой Obsidian vault находится в `~/Documents/Obsidian/MyVault`. Выполни `npx -y obsidian-mcp-rs install claude-code ~/Documents/Obsidian/MyVault` (для других клиентов — `cursor`, `windsurf`, `vscode`, `claude`, …). Инсталлер копирует сервер в постоянное место и прописывает этот путь в мой конфиг — скажи, куда он его положил, и скажи, написал ли он, что сервер будет обновляться сам. Затем напомни перезапустить сессию и подтвердить сервер, если клиент попросит.
...
**Сервер обновляется сам, начиная с 0.8.0.** После того как его поставил `install`, он не чаще раза в сутки проверяет, вышел ли новый релиз, и в фоне заменяет себя, если вышел.

> Авто-обновление появилось *в* 0.8.0, поэтому саму 0.8.0 оно принести не может — копия постарше про него не знает. Выполните `npx obsidian-mcp-rs@latest install` один раз, дальше он справится сам. `npx obsidian-mcp-rs list` скажет, если ваша копия старее. От вас ничего не требуется, и ничего не прерывается — новую версию клиент запустит **при следующем старте**.

Четыре вещи, которых он сознательно не делает:

- **Ждёт 48 часов после выхода релиза.** Если релиз окажется сломанным, именно в этом окне его отзывают — и до вас он не доедет.
- **Заменяет только ту копию, которую положил `install`.** Бинарник из `cargo install`, из пакета дистрибутива или тот, что кеширует `npx`, принадлежит тому, кто его туда положил. Такие копии себя не обновляют.
- **Ходит в сеть не чаще раза в сутки на машину**, сколько бы раз клиент ни перезапускал сервер, а неудачная проверка проходит молча — машина без сети или за файрволом от неё не тормозит и не ломается.
- **Проверяет то, что скачал**, по контрольной сумме, опубликованной вместе с релизом, и один раз запускает новый бинарник перед установкой. Всё, что отвечает не той версией, которую обещало, выбрасывается.

Выключается это через установку — именно там ответ и записывается:

```bash
npx obsidian-mcp-rs@latest install claude ~/vault --no-auto-update
```

Клиента и хранилище нужно назвать. Голый `install --no-auto-update` запускает интерактивный визард, а визард записывает ответ только после того, как клиент выбран, — так что выход из визарда ничего не меняет. Визард спрашивает, а не решает за вас, и по умолчанию предлагает текущий выбор. Ответ запоминается; передумать — та же команда с `--auto-update`. Установка, которая про авто-обновление ничего не говорит, ничего и не меняет: настройка второго клиента не включит его обратно.

Взять релиз сразу, не дожидаясь проверки, нужно **установленной копией**: `update` действует на тот бинарник, который его запустил, а копия в кеше npm — не та, что прописана в конфигах. Путь печатает `npx obsidian-mcp-rs list`:

```bash
# macOS; Linux и Windows — см. таблицу выше
"$HOME/Library/Application Support/obsidian-mcp-rs/bin/obsidian-mcp-rs" update           # взять последний релиз
"$HOME/Library/Application Support/obsidian-mcp-rs/bin/obsidian-mcp-rs" update --check   # просто сказать, что доступно
"$HOME/Library/Application Support/obsidian-mcp-rs/bin/obsidian-mcp-rs" update --force   # взять, не дожидаясь 48-часовой выдержки
```

**Что уходит при проверке.** Один `GET` на `api.github.com` в сутки — с тем, что несёт любой HTTP-запрос, то есть с вашим IP, и с `User-Agent` вида `obsidian-mcp-rs/<версия>`. Само обновление затем качает бинарник с `github.com`, который редиректит на `githubusercontent.com`. Ничего о ваших хранилищах, заметках, клиенте или машине не передаётся, и ничто не связывает вас между днями сверх того, что GitHub может вывести из адреса. Переменные `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` учитываются. Если и это лишний запрос — `--no-auto-update` выключает его полностью.

Вернуться на более старый релиз — поставить его с выключенным авто-обновлением: `npx obsidian-mcp-rs@0.8.1 install claude ~/vault --no-auto-update`. Без этого флага суточная проверка следует за `releases/latest` в обе стороны и в течение суток вернёт более новый — и это сделано намеренно: отзыв плохого релиза — это перевод его в prerelease, то есть исчезновение из `releases/latest`, и все, кто его уже взял, должны иметь возможность откатиться. Путь в конфигах не меняется никогда, поэтому перенастраивать ничего не нужно ни в ту, ни в другую сторону. (Одно исключение, и оно касается всех, кто ставился рано: в конфиге, созданном до 0.7.0, этого пути ещё нет, так что до него ничего из перечисленного не доходит — см. [ниже](#обновление-конфига-созданного-до-070).)

`npx obsidian-mcp-rs list` показывает, где лежит сервер, какой он версии и включено ли авто-обновление.

**На Windows перед запуском `install` закройте AI-клиенты.** Windows не даёт перезаписать запущенный исполняемый файл; если клиент всё ещё держит сервер открытым, инсталлер скажет об этом и попросит закрыть клиент. (`update` это не касается — он переименовывает, а не перезаписывает.)
...
- **Осиротевшие MCP-процессы вообще.** Некоторые клиенты не завершают stdio MCP-серверы при нештатном выходе ([#22612](https://github.com/anthropics/claude-code/issues/22612), [#1935](https://github.com/anthropics/claude-code/issues/1935), [#40667](https://github.com/anthropics/claude-code/issues/40667)). Этот сервер следит за процессом, который его запустил, и выходит вместе с ним, на всех трёх платформах: опрос `getppid` на macOS и Linux, ожидание на хэндле родительского процесса на Windows.
