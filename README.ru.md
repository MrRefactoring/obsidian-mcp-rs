...
**Сервер обновляется сам.** После того как его поставил `install`, он не чаще раза в сутки проверяет, вышел ли новый релиз, и в фоне заменяет себя, если вышел. От вас ничего не требуется, и ничего не прерывается — новую версию клиент запустит **при следующем старте**.
